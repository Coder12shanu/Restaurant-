...
namespace TequliasRestaurant.Controllers
{
    public class OrderController : Controller
    {
        ...
        [HttpPost]
        [Authorize]
        public async Task<IActionResult> PlaceOrder(Product product)
        {
            var productId = 0;
            var model = HttpContext.Session.Get<OrderViewModel>("OrderViewModel");
            if (model == null || model.OrderItems.Count == 0)
            ...
            var stock = model.Products.Where(i => i.ProductId == productId).FirstOrDefault().Stock;
            ...
            var existingProduct = await _products.GetByIdAsync(productId, new QueryOptions<Product> { Includes = "ProductIngredients" });

            var ptoductQuantity = order.OrderItems.FirstOrDefault().Quantity;

            existingProduct.Stock = stock - ptoductQuantity;
