...
namespace TequliasRestaurant.Controllers
{
    ...
    {
        private const bool isNewImg = false;
        private Repository<Product> products;
        private Repository<Ingredient> ingredients;
        private Repository<Category> categories;
        private readonly IWebHostEnvironment _webHostEnvironment;
        private readonly IMemoryCache _cache;  // Cache object

        private string imgFile;

        public ProductController(ApplicationDbContext context, IWebHostEnvironment webHostEnvironment, IMemoryCache cache)
        {
        ...
        public async Task<IActionResult> AddEdit(int id)
        {
            ...
            else
            {
                Product product = await products.GetByIdAsync(id, new QueryOptions<Product>
                {
                    ...
                    _cache.Set("CachedImageUrl", product.ImageUrl);
        ...
        [HttpPost]
        public async Task<IActionResult> AddEdit(Product product, int[] ingredientIds, int catId, bool isNewImg)
        {
            ViewBag.Ingredients = await ingredients.GetAllAsync();
            ViewBag.Categories = await categories.GetAllAsync();
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    if (_cache.TryGetValue("CachedImageUrl", out string cachedImageUrl) && !isNewImg)
                    {
                        existingProduct.ImageUrl = cachedImageUrl;
                    }
                    else
                    {
